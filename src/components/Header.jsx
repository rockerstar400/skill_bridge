...
import { NavHashLink } from 'react-router-hash-link'; // Smooth scroll ke liye
import logo from "../assets/logo.png"
import { Menu, X, ArrowRight } from 'lucide-react';
...
const Header = () => {
    ...
    // Updated Navigation: Careers alag page hai, baaki sections hain
    const navLinks = [
        { name: 'Home', path: '/#home' },
        { name: 'About', path: '/#about' },
        { name: 'Services', path: '/#services' },
        { name: 'Careers', path: '/careers' }, // Naya path
        { name: 'Contact', path: '/#contact' }
    ];
    ...
    return (
        <nav className={`fixed top-0 w-full z-[100] transition-all duration-700 ease-in-out ${isScrolled
            ...
            <div className="max-w-7xl mx-auto px-6 lg:px-12 flex justify-between items-center">
                ...
                <NavHashLink smooth to="/#home" className="flex items-center cursor-pointer group">
                    <motion.div
                        initial={{ opacity: 0, x: -20 }}
                        animate={{ opacity: 1, x: 0 }}
                        whileHover={{ scale: 1.05 }}
                ...
                <div className="hidden md:flex items-center space-x-10">
                    <div className="flex items-center space-x-8">
                        {navLinks.map((link) => {
                            // Active logic: Career page pe careers highlight hoga, home sections pe home sections
                            const isActive = (location.pathname + location.hash) === link.path || 
                                             (link.path === '/#home' && location.pathname === '/' && location.hash === '');
